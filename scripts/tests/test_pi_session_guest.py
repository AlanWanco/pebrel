...
class GuestSessionMetadataTests(unittest.TestCase):
    def setUp(self):
        ...
        self.config.mkdir()
        self.env = {**os.environ, 'PI_CODING_AGENT_DIR': str(self.config)}
    ...
    def run_script(self, exact=''):
        result = subprocess.run(['sh', str(SCRIPT), SESSION, str(exact)], cwd=self.cwd,
    ...
    def test_exact_file_does_not_require_default_directory_or_json_parser(self):
        ...
        self.env['PI_CODING_AGENT_DIR'] = str(self.root / 'missing-config')
        result = self.run_script(path)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout.split('\t')[0], str(path))
    ...
    def test_environment_override_and_missing_exact_path_search_native_headers(self):
        ...
        self.env['PI_CODING_AGENT_SESSION_DIR'] = str(directory)
