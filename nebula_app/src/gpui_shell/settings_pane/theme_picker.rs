...
pub(super) fn theme_sample(name: ThemeName, token: bool, compact: bool) -> gpui::Div {
    ...
    let line = |width, color| {
        div().w(gpui::relative(width)).h(ui(if token { 3.0 } else { 4.0 })).rounded_full().bg(color)
    };
    v_flex()
        .w_full()
        .h(ui(if token {
        ...
        .child(
            v_flex()
                .size_full()
                .justify_center()
                .gap(ui(if token { 4.0 } else { 6.0 }))
                .p(ui(if token { 5.0 } else { 8.0 }))
                .rounded(ui(0.0))
...
impl SettingsPane {
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(div().text_size(ui(9.0)).text_color(ink.opacity(0.6)).child("PREVIEW")),
            )
            .child(
                v_flex()
                    .id(if typography {
                    ...
                    .w_full()
                    .min_h(ui(if compact { 110.0 } else { 181.0 }))
                    .px(ui(if typography { 16.0 } else { 13.0 }))
                    .py(ui(if compact { 12.0 } else { 19.0 }))
                    .overflow_x_scroll()
                    ...
                    .child(div().mt(ui(10.0)).text_size(px(size * 0.75)).child(format!(
                        "{} · {}",
                        crate::brand::NAME,
                        std::env::consts::OS
                    )))
                    .child(
                        h_flex()
                            ...
                            .child(Icon::new(IconName::Check).size(ui(12.0)).text_color(accent))
                    ...
                    .child(
                        h_flex()
                            ...
                            .gap(ui(7.0))
                            .child(div().text_color(accent).child("❯"))
                            .child(div().w(ui(7.0)).h(ui(13.0)).bg(ink.opacity(0.85))),
                    ),
            )
    }
    ...
    pub(super) fn theme_picker_body(
        ...
        let grid_width = if compact { width } else { width - (230.0 + 25.0) * scale };
        let gap = if compact { 6.0 } else { 9.0 };
        let option_width = (grid_width - (columns - 1) as f32 * gap * scale) / columns as f32;
        let grid = h_flex()
                ...
                let content = v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .justify_between()
                            .gap(ui(3.0))
                            .text_size(ui(if compact { 10.0 } else { 11.5 }))
        ...
        let preview_width = if compact { px(width) } else { px(230.0 * scale) };
        let preview = v_flex().w(preview_width).flex_shrink_0()
            .child(self.terminal_appearance_preview(draft, false, compact, window, cx))
            .child(div().mt(ui(if compact { 11.0 } else { 19.0 })).text_size(ui(13.0)).font_semibold()
                .when(!compact, |title| title.text_center()).child(chrome_theme(draft).short_label()))
            .child(div().mt(ui(5.0)).text_size(ui(10.5)).text_color(colors.secondary).when(!compact, |text| text.text_center())
                .child(if palette.is_light { language.pick("浅色主题", "Light theme") } else { language.pick("深色主题", "Dark theme") }))
            .when(!compact, |preview| preview.child(h_flex().mt(ui(20.0)).pt(ui(17.0)).justify_center().gap(ui(9.0))
                .border_t_1().border_color(colors.line).children(swatches.into_iter().enumerate().map(|(index, (color, label))| {
                    ...
                    div().id(("theme-color-swatch", index)).size(ui(24.0)).rounded_full().border_1()
            ...
            .child(div().mt(ui(if compact { 9.0 } else { 20.0 })).text_size(ui(10.5))
