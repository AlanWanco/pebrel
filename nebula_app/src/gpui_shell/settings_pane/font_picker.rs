...
impl SettingsPane {
    ...
    pub(super) fn font_picker_dropdown(
        ...
        let panel_width = px((FONT_PICKER_PANEL_WIDTH * scale).min(
            (f32::from(viewport.width) - 2.0 * FONT_PICKER_WINDOW_MARGIN * scale).max(0.0),
        ));
        let (open_above, list_height) = trigger_bounds
            ...
            .map(|bounds| {
                let below = (f32::from(viewport.height - bounds.bottom_right().y)
                    ...
                    .max(0.0);
                let above = (f32::from(bounds.origin.y)
                    - FONT_PICKER_OFFSET_Y * scale
                    - FONT_PICKER_WINDOW_MARGIN * scale)
                    .max(0.0);
                let preferred =
                    (FONT_PICKER_PANEL_CHROME_HEIGHT + FONT_PICKER_LIST_PREFERRED_HEIGHT) * scale;
                let open_above = below < preferred && above > below;
                let available = if open_above { above } else { below };
                (
                    open_above,
                    px((available - FONT_PICKER_PANEL_CHROME_HEIGHT * scale)
                        .clamp(0.0, FONT_PICKER_LIST_PREFERRED_HEIGHT * scale)),
                )
            })
            .unwrap_or((false, px(FONT_PICKER_LIST_PREFERRED_HEIGHT * scale)));
        let panel =
            self.font_picker_open.then(|| self.font_picker_panel(panel_width, list_height, cx));

        div().relative().w(ui(SETTINGS_SELECT_WIDTH)).flex_shrink_0().child(row).when_some(
...
mod interaction_tests {
    ...
    fn review_regression_font_fields_align_and_dropdown_toggles_with_search(
        ...
        let expected_font = runtime
            .font_family
            .clone()
            .unwrap_or_else(|| REQUIRED_FONT_FAMILY.to_owned());
        let expected_cjk = runtime
            .font_family_cjk
            .clone()
            .unwrap_or_else(|| REQUIRED_FONT_FAMILY.to_owned());
        cx.update(|cx| {
            gpui_component::init(cx);
            let mut settings = crate::gpui_shell::config::Settings::load(
                nebula_settings::ThemeName::Nord,
            );
            // Tests must not inherit the developer's persisted UI scale.
        ...
        let (_, cx) = cx.add_window_view(|window, cx| {
            // `ui(...)` dimensions use the product's 14px root REM; the test
            // harness otherwise starts with gpui-component's 16px default.
